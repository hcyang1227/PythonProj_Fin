# OpenCV-Python使用DNN模块实现人脸识别和人脸检测
# https://mdnice.com/writing/51f9f2f4232a47f2afc32dc46e5472eb
# 【他山之石】详细记录insightface的SCRFD人脸检测ncnn实现
# SCRFD 是高效率高精度人脸检测算法，2021年5月刚发出来的，速度和精度相比其他算法都有提升！
# https://toutiao.io/posts/6kd34s9/preview
...
class SCRFD():
        ...
        if self.keep_ratio and srcimg.shape[0] != srcimg.shape[1]:
            ...
            if hw_scale > 1:
                ...
                img = cv2.copyMakeBorder(
                    img, padh, self.inpHeight - newh - padh, 0, 0, cv2.BORDER_CONSTANT, value=0)
        else:
            img = cv2.resize(
    ...
    def detect(self, srcimg):
        ...
        blob = cv2.dnn.blobFromImage(img, 1.0/128, (self.inpWidth, self.inpHeight), (127.5, 127.5, 127.5), False, False)
        ...
        for i in indices:
            ...
            cv2.rectangle(srcimg, (xmin, ymin), (xamx, ymax),
                          (0, 0, 255), thickness=2)
            for j in range(5):
                cv2.circle(srcimg, (int(kpss[i, j, 0]), int(
                    kpss[i, j, 1])), 1, (0, 255, 0), thickness=-1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--onnxmodel', default='SCRFD_500M_KPS_640x640.onnx', type=str, choices=['SCRFD_500M_KPS_640x640.onnx', 'SCRFD_2.5G_KPS_640x640.onnx', 'SCRFD_10G_KPS_640x640.onnx'], help='onnx model')
